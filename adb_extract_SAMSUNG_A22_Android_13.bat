...
:: ########################## MAIN ############################
:: ############################################################
:: Debut du script et des variables
...
:: ####################### FUNCTION ############################
:: ########## Fonction set_var - Definition variables ##########
:: #############################################################
:set_var
...
:: #########################################################################
:: ########## Fonction start_adb_server - Lancement du server ADB ##########
:: #########################################################################
:start_adb_server
...
timeout /t 5
%ADB% start-server
echo [INFO] En attente de la validation de connexion USB sur le smartphone
pause 
...
:: ########################################################################
:: ########## Fonction check_device - Controle des peripheriques ##########
:: ########################################################################
:check_device

echo [INFO] Controle de la connexion USB
:: Android ID
for /f %%A in ('%SHELL_COMMAND% getprop ro.serialno') do set "ANDROID_SERIAL_NUMBER=%%A"
...
:: ##############################################################################
:: ########## Fonction set_path - Definition des chemins de sauvegarde ##########
:: ##############################################################################
:set_path

echo [INFO] Definition des chemins de sauvegarde

:: Chemins génériques
set "SPATH=%CLEANED_ANDROID_ID%"
MKDIR "%SPATH%"

:: Répertoires pour les informations de l'appareil
set "INFO_DIR=%SPATH%\info"
set "INFO_TXT_FILE=%INFO_DIR%\device_info.txt"
mkdir "%INFO_DIR%"

:: Répertoires pour l'exécution de commandes en direct
set "LIVE_DIR=%SPATH%\live"
set "LIVE_LOG_FILE=%LIVE_DIR%\log_live_acquisition.txt"
mkdir %LIVE_DIR%

:: Répertoires pour l'exécution du gestionnaire de paquets
set "PM_DIR=%SPATH%\package_manager"
set "PM_LOG_FILE=%PM_DIR%\log_pm_acquisition.txt"
mkdir %PM_DIR%

:: Répertoires pour l'acquisition DUMPSYS
...
:: ########################################################################################
:: ########## Fonction info_collect - Collecte des informations matériel systeme ##########
:: ########################################################################################
:info_collect
...
for /f %%A in ('%SHELL_COMMAND% "service call iphonesubinfo 1 s16 com.android.shell | cut -c 52-66 | tr -d '.[:space:]'"') do set "IMEI=%%A"
...
for /f %%A in ('%SHELL_COMMAND% getprop ro.hardware') do set "CHIPNAME=%%A"
...
:: #####################################################################################
:: ########## Fonction live_collect - Collecte d'informations supplémentaires ##########
:: #####################################################################################
:live_collect
...
%SHELL_COMMAND% cat /proc/partitions > "%LIVE_DIR%\partitions.txt" 2> "%LIVE_DIR%\proc_part_log.txt"
...
:: ####################################################################################
:: ########## Fonction package-manager - Collecte d'informations sur les APK ##########
:: ####################################################################################
:package_manager
...
:: ################################################################################
:: ########## Fonction sd_card - Collecte d'informations sur la carte SD ##########
:: ################################################################################
:sd_card
...
::calcul de hash et creation archive a integrer
...
:: ###########################################################################################
:: ########## Fonction info_sys - Collecte d'informations sur le système de fichier ##########
:: ###########################################################################################
:info_sys
...
:: #####################################################################
:: ########## Fonction full_copy - Copie de tout le téléphone ##########
:: #####################################################################
:full_copy
...
:: ###############################################################################
:: ########## Fonction adb_backup - Lancement d'une sauvegarde complète ##########
:: ###############################################################################
:adb_backup
...
echo %SHELL_COMMAND% input tap 850 2200 >> %TMP_SCRIPT%
...
:: #########################################################################################
:: ########## Fonction user_data_extract - Collecte des informations utilisateurs ##########
:: #########################################################################################
:user_data_extract
...
:: #####################################################
endlocal  
:: Fin du script et des variables
:: ####################### FIN #########################