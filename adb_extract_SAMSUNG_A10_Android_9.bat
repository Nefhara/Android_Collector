...
for /f %%A in ('%SHELL_COMMAND% getprop ro.hardware.chipname') do set "CHIPNAME=%%A"
...
%SHELL_COMMAND% cat /proc/sched_debug > "%LIVE_DIR%\sched_debug.txt" 2> "%LIVE_DIR%\proc_sched_log.txt"
%SHELL_COMMAND% sysctl -a > "%LIVE_DIR%\sysctl.txt" 2> "%LIVE_DIR%\sysctl_log.txt"
...
echo %SHELL_COMMAND% input tap 550 1400 >> %TMP_SCRIPT%
